...
StatusType CoursesManager::WatchClass(int courseID, int classID, int time) {
    ...
    AVLNode<ViewData>* recent_lecture = watched_lecture_tree.insert_value(ViewData(courseID, classID, old_time_viewed + time));
    temp_node->data.lectures[classID].add_time(time);

    if(strongest_lecture == nullptr){
        strongest_lecture = recent_lecture;
    }else if(strongest_lecture->data < recent_lecture->data)
        strongest_lecture = recent_lecture;
